...
function findSponsorBySlug(slug: string): Sponsor | undefined {
  return sponsors.find((s) => slugify(s.name) === slug);
}

const tierConfig = {
  [SPONSOR_TIERS.TITLE]: {
    styles:
      "border-ted-red/30 bg-ted-red/[0.05] shadow-[0_0_50px_rgba(235,0,40,0.1)]",
  ...
  [SPONSOR_TIERS.SILVER]: {
    ...
    cardWidth:
      "w-[calc(50%-0.5rem)] sm:w-[calc(33.33%-0.75rem)] max-w-sm min-h-[180px] sm:min-h-[220px]",
  ...
  [SPONSOR_TIERS.BRONZE]: {
    ...
    span: "col-span-1 md:col-span-2 lg:col-span-2",
    cardWidth:
      "w-[calc(50%-0.5rem)] sm:w-[calc(33.33%-0.75rem)] lg:w-[calc(25%-0.75rem)] min-h-[160px] sm:min-h-[180px]",
        ...
        <Container>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6 auto-rows-max">
            ...
            {sponsorTiers.map((tier) => {
              ...
              return (
                <AnimatedSection key={tier} className="col-span-full mb-8">
                  {/* Tier Label */}
                  ...
                  <div className="flex flex-wrap justify-center gap-4 md:gap-6">
                    {tierSponsors.map((sponsor) => (
                      ...
                      >
                        ...
                        <div
                          className={`relative w-full ${config.imageClass} z-10 transition-all duration-500 group-hover:scale-105 flex items-center justify-center`}
                        >
                            ...
                            {sponsor.logo && (
                              <Image
                                ...
                                className={`object-contain transition-all duration-500 ${
                                  sponsor.name.toLowerCase().includes("ather")
                                    ? "invert brightness-[2] opacity-80 group-hover:opacity-100"
            ...
            <AnimatedSection className="col-span-full mt-12 md:mt-16">
              ...
              <div className="flex flex-wrap justify-center gap-4 md:gap-6">
                {sponsors
                  .filter((s) => s.tier === SPONSOR_TIERS.COMMUNITY)
                  .map((sponsor) => (
                    <button
                      key={sponsor.name}
                      onClick={() => openSponsor(sponsor)}
                    ...
                    >
                      <div
                        className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${tierConfig[SPONSOR_TIERS.COMMUNITY].tint}`}
                      />
                      <div className="relative w-full flex-1 px-6 z-10 transition-all duration-700 group-hover:scale-105">
                        {sponsor.logo && (
                          <div
                            className={`relative w-full ${tierConfig[SPONSOR_TIERS.COMMUNITY].imageClass}`}
                          >
                            <Image
                              src={sponsor.logo}
              ...
              <div className="w-full md:w-[45%] lg:w-[40%] bg-black flex items-center justify-center p-12 md:p-16 relative overflow-hidden shrink-0 border-b md:border-b-0 md:border-r border-white/5">
                ...
                >
                    ...
                    {selectedSponsor.logo && (
                      <Image
                        ...
                        className={`object-contain filter drop-shadow-[0_20px_40px_rgba(0,0,0,0.4)] ${
                          selectedSponsor.name.toLowerCase().includes("ather")
                            ? "invert brightness-[2]"
                            : ""
                        }`}
                  ...
                  >
                    ...
                    <div
                      className={`h-px w-12 mx-auto mt-3 ${tierConfig[selectedSponsor.tier as keyof typeof tierConfig].accent} opacity-50`}
                    />
                  </motion.div>
                </motion.div>
              </div>
              ...
              <div className="w-full md:w-auto flex-1 p-8 md:p-12 lg:p-16 flex flex-col justify-center relative overflow-hidden">
                ...
                >
                  <div className="flex flex-wrap items-center gap-3 mb-6">
                    <span
                      className={`text-[10px] font-[900] uppercase tracking-[0.2em] text-white px-4 py-1.5 ${tierConfig[selectedSponsor.tier].accent} rounded-full shadow-lg`}
                    >
...
export default function SponsorsPage() {
  return (
    <Suspense fallback={<SponsorsPageFallback />}>
      <SponsorsPageInner />
    </Suspense>
  );
}

function SponsorsPageFallback() {
  return (
    <>
      <PageHero
        title="Our Partners"
        ...
        <Container>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6 auto-rows-max">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="col-span-full mb-8 w-full h-56 md:h-72 bg-white/[0.03] rounded-[2rem] border border-white/[0.05] animate-pulse"
              />
            ))}
          </div>
        </Container>
